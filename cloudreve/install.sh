...
exit_install() {
	...
	1)
		...
		echo_date "退出安装!"
		rm -rf /tmp/cloudreve* >/dev/null 2>&1
		exit 1
		;;
	2)
		echo_date "Cloudreve插件目前仅支持hnd机型中的armv8机型!"
		echo_date "你的路由器不能安装!!!"
		echo_date "退出安装!"
		rm -rf /tmp/cloudreve* >/dev/null 2>&1
		exit 1
...
install_now() {
	...
	local DESCR="一个公私兼备的网盘系统，使用 Go 和 DBMS。"
	local PLVER=$(cat ${DIR}/version)

	# stop signdog first
	enable=$(dbus get cloudreve_enable)
	if [ "${enable}" == "1" -a "$(pidof cloudreve)" ]; then
		echo_date "先关闭cloudreve插件!以保证更新成功!"
	...
	dbus set cloudreve_old_dir=/koolshare/bin

	# 检查插件默认dbus值
	dbus_nset cloudreve_port "5212"
	dbus_nset cloudreve_cert_file "/etc/cert.pem"
	dbus_nset cloudreve_key_file "/etc/key.pem"

	# reenable
	if [ "${enable}" == "1" ]; then
		echo_date "重新启动cloudreve插件!"
		sh /koolshare/scripts/cloudreve_config.sh boot_up
	fi
